...
class Cx extends TagLib
{
    ...
    protected $tags = [
        ...
        'uri'        => ['attr' => 'link,vars,suffix,domain', 'close' => 0, 'expression' => true],
    ...
    public function tagForeach($tag, $content)
    {
        ...
        $parseStr .= 'if(isset('.$name.') && is_array(' . $name . ') ): ';
     ...
     * @return string
     */
    public function tagDefault($tag,$content)
    {
    ...
    public function tagUrl($tag, $content)
    {
        ...
        $url    = $tag['link'] ?? '';
        $vars   = $tag['vars'] ?? '';
        $suffix = $tag['suffix'] ?? 'true';
        $domain = $tag['domain'] ?? 'false';
