...
use Spartan\Driver\Uploader\File;
...
class Request
{
    ...
    /**
     * 构造一个基础上传文件类
     * @param string $fileField 表单名称。若开启base64编码，则$fileField代表的是base64编码的字符串表单名
     * @return \Spartan\Driver\Uploader\UpFile
     */
    public function UpFile($fileField){
        return new \Spartan\Driver\Uploader\UpFile($fileField);
    }

    public function init(array $options = [])
    {
        $this->config = array_merge($this->config,$options);
        $strDomainRoot = config('SITE.DOMAIN_ROOT');
        $strDomainRoot && $this->config['url_domain_root'] = $strDomainRoot;
    ...
    public function __call($method, $args)
    {
        ...
        \Spt::halt('method not exists:' . static::class . '->' . $method);
        return false;
    }
    ...
    public function subDomain()
    {
        if (is_null($this->subDomain)) {
            $rootDomain = $this->config['url_domain_root'];// 获取当前主域名
            if ($rootDomain) {
                // 配置域名根 例如 baidu.com 163.com.cn 如果是国家级域名 com.cn net.cn 之类的域名需要配置
                $domain = explode('.', rtrim(stristr($this->host(true), $rootDomain, true), '.'));
            } else {
                $domain = explode('.', $this->host(true));
            }
    ...
    public function cookie($name = '', $default = null, $filter = '')
    {
        if (empty($this->cookie)) {
            $this->cookie = Cookie::instance()->get();
        }

        if (!empty($name)) {
            $data = Cookie::instance()->has($name) ? Cookie::instance()->get($name) : $default;
        } else {
            $data = $this->cookie;
        }
     ...
     * @return null|array|\Spartan\Driver\Uploader\File
     */
        ...
        switch (strtolower($type)) {
            ...
            case 's':
                if (is_scalar($data)) {
                    ...
                    \Spt::halt('variable type error：' . gettype($data));
                }
                break;
        }
        return $data;
    ...
    public function token($name = '__token__', $type = null)
    {
        ...
        Session::instance()->set($name, $token);
        return $token;
    }
