...
class Session
{
    ...
    public function init(array $config = [])
    {
        ...
        $_config = array_change_key_case(config('get.SESSION'),CASE_LOWER);
        $config = array_merge($this->config,$_config,$config);
        $isDoStart = false;
        if (isset($config['use_trans_sid']) && $config['use_trans_sid']) {
        ...
        $strSID = $_SERVER['HTTP_X_TOKEN'] ?? ($_REQUEST['php_id'] ?? ($_SERVER['HTTP_PHP_ID'] ?? ''));
        !$strSID && $strSID = $_REQUEST[$config['var_session_id']] ?? ($config['id']??'');
        if (strlen($strSID) > 10) {
            session_id($strSID);
        }
        ...
        if (isset($config['use_cookies']) && $config['use_cookies']) {
            ini_set('session.use_cookies', $config['use_cookies'] ? 1 : 0);
        }
        if (isset($config['cache_limiter']) && $config['cache_limiter']) {
            session_cache_limiter($config['cache_limiter']);
        }
        if (isset($config['cache_expire']) && $config['cache_expire']) {
        ...
        if (!empty($config['type'])) {
            ...
            $class = false !== strpos($config['type'], '\\')?
                $config['type']:
                '\\Spartan\\Driver\\Session\\' . ucwords($config['type']);
            // 检查驱动类
            if (!class_exists($class) || !session_set_save_handler(new $class($config))) {
                \Spt::halt('error session handler:' . $class, $class);
            }
        }
        if ($isDoStart) {
            $this->start();
