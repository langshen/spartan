...
class Template
{
    ...
    {
        $this->config['cache_path'] = APP_ROOT.'Runtime'.DS.'Cache'.DS.config('SUB_APP_NAME').DS;
        ...
        $this->storage = \Spartan\Driver\Template\File::instance();
    }
    ...
    public function parseTagLib($tagLib, &$content, $hide = false)
    {
        if (false !== strpos($tagLib, '\\')) {
            ...
            $className = '\\Spartan\\Driver\\Template\\' . ucwords($tagLib);
        }
        /** @var \Spartan\Driver\Template\Cx $tLib */
                ...
                if (isset($_varParseList[$match[0]])) {
                    ...
                    if (strpos($match[0], '.')) {
                        ...
                        if ('$Spt' == $first) {
                            ...
                            $parseStr = $this->parseSptVar($vars);
                            ...
                            $parseStr = 'request()->' . $method . '(' . $params . ')';
                        } else {
     ...
     * @return string
     */
    public function parseSptVar($vars)
    {
        ...
        if ($vars) {
            switch ($type) {
                case 'SERVER':
                    $parseStr = 'request()->server(\'' . $param . '\')';
                    break;
                case 'GET':
                    $parseStr = 'request()->get(\'' . $param . '\')';
                    break;
                case 'POST':
                    $parseStr = 'request()->post(\'' . $param . '\')';
                    break;
                case 'COOKIE':
                    $parseStr = 'cookie(\'' . $param . '\')';
                    break;
                case 'SESSION':
                    $parseStr = 'session(\'' . $param . '\')';
                    break;
                case 'ENV':
                    $parseStr = 'request()->env(\'' . $param . '\')';
                    break;
                case 'REQUEST':
                    $parseStr = 'request()->request(\'' . $param . '\')';
                    break;
                case 'CONST':
            ...
            switch ($type) {
                ...
                case defined($type):
                    $parseStr = $type;
                    break;
                case function_exists($type) && is_callable($type):
                    $parseStr = $type.'()';
                    break;
                default:
                    if (defined($type)) {
    ...
    private function parseTemplateFile($template)
    {
        ...
        substr($template,0,2) == '..' && $template = substr($template,3);
        !$template && $template = strtolower(\Spt::$arrConfig['ACTION']);
        if(strpos($template,'@') > 0 || strpos($template,'/') > 0 ){//跨模块调用模版文件
            $template = str_replace(['@','/'],[DS,DS],$template);
        }else{
            $template = \Spt::$arrConfig['CONTROL'].DS.$template;
        }
        ...
        if (is_file($template)) {
            ...
            \Spt::halt($template,'template file not exiting');
            return '';
        }
    }
